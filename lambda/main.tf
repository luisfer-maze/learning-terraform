...
resource "null_resource" "function_binary" {
  ...
  provisioner "local-exec" {
    ...
    command = "wget https://dl.google.com/go/go1.21.4.linux-amd64.tar.gz && tar -C /usr/local -xzf go1.21.4.linux-amd64.tar.gz && export PATH=$PATH:/usr/local/go/bin && go version"
  }
